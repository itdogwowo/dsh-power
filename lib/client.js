...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function PowerRow() {
			...
			/**
			 * The browser's own view of network availability. DSH suspends its
			 * connection retries while `navigator.onLine` is false, so a machine
			 * with no ordinary interface (Wi-Fi off, tunnel-only) leaves the GUI
			 * permanently disconnected even though this loopback service is fine.
			 */
			const offline = typeof window.navigator === 'object' && window.navigator !== null
			...
			if (awaitingPid !== null) {
				...
				actions = ['restart', 'shutdown'].map((action) => React.createElement('button', {
					...
					disabled: busy || finished || sessionOk === false || offline,
					title: sessionOk === false
						? '這個頁面沒有有效登入，重啟後不會自動回來'
						: (offline ? '作業系統回報沒有網路，DSH 會暫停重連；重啟後這個頁面不會自己回來' : undefined),
			...
			const offlineHint = offline
				? '作業系統回報沒有網路，DSH 因此暫停重連；接上任何網路（Wi-Fi 或乙太網路）後會自動恢復'
				: null;
