...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const CSS = [
			...
			'.dshpw-spinner { width: 14px; height: 14px; flex: none; border-radius: 50%; border: 2px solid currentColor; border-top-color: transparent; animation: dshpw-spin 0.7s linear infinite; }',
			...
			const run = (action) => {
				...
				requestWithin(ACTION_URL, { action: action }).then((value) => {
					...
					if (value !== null && value !== undefined && typeof value === 'object' && value.ok === true) {
						...
						setNotice({ kind: 'err', text: '操作失敗：等待服務回應逾時，已取消顯示。' });
					} else {
			...
			const actions = pending === null
				...
				: [
					...
					React.createElement('button', {
						...
						? [
							React.createElement('span', { key: 'spinner', className: 'dshpw-spinner', 'aria-hidden': 'true' }),
							'處理中…',
						]
						: '確認'),
					React.createElement('button', {
