...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/**
		 * Repair a stale browser network flag.
		 *
		 * DSH suspends its connection retries while `navigator.onLine` is false
		 * and only resumes on the browser's `online` event. A browser can leave
		 * that flag false long after the machine is reachable again (macOS
		 * reports Reachable, ordinary HTTP works, this loopback service answers —
		 * and the page still insists it is offline). The result is a GUI that
		 * asks to reconnect forever, for a reason no server-side fix can touch.
		 *
		 * This is called only when this page has just proven that the local
		 * service answers, so it corrects a stale flag rather than inventing
		 * connectivity.
		 *
		 * @returns true when the flag was overridden.
		 */
		const repairOfflineFlag = () => {
			try {
				const navigatorObject = window.navigator;
				if (navigatorObject === undefined || navigatorObject === null) return false;
				if (navigatorObject.onLine !== false) return false;
				Object.defineProperty(navigatorObject, 'onLine', { configurable: true, get: () => true });
				const Constructor = window.Event;
				if (typeof Constructor === 'function') {
					window.dispatchEvent(new Constructor('online'));
				} else if (window.document !== undefined && window.document !== null
					&& typeof window.document.createEvent === 'function') {
					const legacy = window.document.createEvent('Event');
					legacy.initEvent('online', false, false);
					window.dispatchEvent(legacy);
				}
				return true;
			} catch (error) {
				/* a browser that forbids the override keeps its own answer */
				return false;
			}
		};
		...
		/**
		 * Whether this origin is ready to SERVE the page yet.
		 *
		 * A new process starts answering `/api/dsh-power/info` — this plugin's own
		 * route — before the application's `/` handler is mounted, because the
		 * plugin tree comes up first. Navigating on the info answer alone can
		 * therefore land on a server that is listening but has no `/` route yet,
		 * which the browser reports as `HTTP ERROR 404` — a dead end the user has
		 * to refresh out of.
		 *
		 * The auth fence answers `/` as soon as it exists (200, a redirect, or
		 * 401/403 for a page without a session), so any answer except "no such
		 * route" and "server error" counts as ready. A 401 is deliberately ready:
		 * it is the correct answer for a page whose cookie belongs to the process
		 * that just died, and the app's own login screen is what should appear.
		 *
		 * @returns a promise for whether `/` is being served.
		 */
		const pageIsServed = () => {
			if (typeof fetch !== 'function') return Promise.resolve(true);
			return fetch('/', { method: 'GET', credentials: 'same-origin' }).then(
				(response) => response.status !== 404 && response.status < 500,
				() => false,
			);
		};

		/** Wall-clock budget for waiting on the page to be served, in ms. */
		const SERVE_BUDGET_MS = 20000;
		/** How long to let one attempt to `/` take before trying again. */
		const SERVE_ATTEMPT_MS = 4000;

		/**
		 * Wait until the new process serves `/`, then navigate.
		 *
		 * Bounded and self-cancelling: if the page never becomes servable the row
		 * says so instead of leaving the user staring at "重新連線中…", and a tab
		 * that goes away mid-wait stops on its own.
		 *
		 * @param stillAlive - whether this component is still mounted.
		 * @param done - called once navigation has been started, or once waiting failed.
		 */
		const goHomeWhenServed = (stillAlive, done) => {
			const deadline = Date.now() + SERVE_BUDGET_MS;
			const attempt = () => {
				...
				const guard = setTimeout(retry, SERVE_ATTEMPT_MS);
				pageIsServed().then((served) => {
					if (settled) return;
					settled = true;
					clearTimeout(guard);
					if (!stillAlive()) return;
					if (served) { goHome(); done(true); return; }
					if (Date.now() > deadline) { done(false); return; }
					setTimeout(attempt, RECONNECT_POLL_MS);
				}, () => {
					if (settled) return;
		...
		function PowerRow() {
			...
			/**
			 * The browser's view of network availability, re-read after this row
			 * has proven the local service answers. DSH suspends its connection
			 * retries while `navigator.onLine` is false, so a stale flag leaves the
			 * GUI permanently disconnected even though everything here works.
			 */
			const [offlineNow, setOfflineNow] = React.useState(
				typeof window.navigator === 'object' && window.navigator !== null && window.navigator.onLine === false,
			);
			...
			React.useEffect(() => {
				...
				requestWithin(INFO_URL, undefined).then((value) => {
					...
					if (value !== null && value !== undefined && typeof value === 'object' && value.ok === true) {
						...
						// The service answered over loopback: a stale offline flag is
						// now demonstrably wrong, so correct it and re-read it.
						repairOfflineFlag();
					} else {
						setTarget({ pid: '', port: null, command: '' });
					}
					setOfflineNow(typeof window.navigator === 'object' && window.navigator !== null && window.navigator.onLine === false);
				});
				return () => { alive = false; };
			}, []);
			...
			React.useEffect(() => {
				...
				const poll = () => {
					...
					requestWithin(INFO_URL, undefined).then((value) => {
						...
						if (ready) {
							// The new process answers this plugin's route before it
							// serves the page, so wait for `/` instead of navigating
							// straight into a 404.
							goHomeWhenServed(() => alive, (navigated) => {
								if (navigated || !alive) return;
								setNotice({ kind: 'err', text: '服務已重新啟動，但頁面還沒準備好；請重新整理頁面。' });
								setFinished(false);
								setAwaitingPid(null);
							});
							return;
						...
						if (tries > RECONNECT_TRIES) {
							setNotice({ kind: 'err', text: '服務尚未回應，請重新整理頁面。' });
							setFinished(false);
							setAwaitingPid(null);
							return;
						}
			...
			if (awaitingPid !== null) {
				...
				actions = ['restart', 'shutdown'].map((action) => React.createElement('button', {
					...
					disabled: busy || finished || sessionOk === false || offlineNow,
					title: sessionOk === false
						? '這個頁面沒有有效登入，重啟後不會自動回來'
						: (offlineNow ? 'DSH 在瀏覽器回報離線時會暫停重連；重啟後這個頁面不會自己回來' : undefined),
			...
			const sessionWarning = sessionOk === false
				...
				: null;
			/** Explains the app's own reconnect prompt instead of leaving it a mystery. */
			const offlineHint = offlineNow
				? '瀏覽器回報離線，DSH 因此暫停重連；若系統其實有網路，請重啟瀏覽器以更新該狀態'
				: null;
			const banner = sessionWarning !== null ? sessionWarning : offlineHint;
			const desc = banner !== null ? banner : (notice !== null ? notice.text : idle);
			const hover = target !== null && target.pid !== ''
				? 'PID ' + target.pid + ' — ' + target.command
				: undefined;

			const descClass = sessionWarning !== null || (banner !== null && notice === null)
				? (sessionWarning !== null ? 'dshpw-desc dshpw-desc-err' : 'dshpw-desc dshpw-desc-err')
				: notice === null
					? 'dshpw-desc'
		...
		function apply(ctx) {
			...
			// The local service answering is proof enough to distrust a stale
			// offline flag — see repairOfflineFlag().
			fetch(INFO_URL, { method: 'GET', cache: 'no-store' }).then(
				(response) => {
					if (!response.ok) return;
					if (repairOfflineFlag()) {
						report({ kind: 'network', online: true, repaired: true, note: 'stale navigator.onLine overridden; online dispatched' });
					} else {
						report({ kind: 'network', online: typeof window.navigator === 'object' && window.navigator !== null ? window.navigator.onLine : null, repaired: false });
					}
				},
				() => undefined,
			);
